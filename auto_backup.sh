...
# ------------------------------------------------------------------
# 1️⃣ Дамп базы в custom-формате
# ------------------------------------------------------------------
PG_DUMP_FILE="$BACKUP_DIR/${DB_NAME}_$DATE.dump"
# Меняем рабочий каталог на /tmp, чтобы pg_dump не ругался на домашний каталог
cd /tmp || exit
sudo -u $PG_USER pg_dump -Fc -f "$PG_DUMP_FILE" "$DB_NAME"
echo "[$(date)] Database dump saved to $PG_DUMP_FILE"

# ------------------------------------------------------------------
# 2️⃣ Дамп ролей и глобальных объектов
# ------------------------------------------------------------------
PG_ROLES_FILE="$BACKUP_DIR/roles_$DATE.sql"
sudo -u $PG_USER pg_dumpall --globals-only > "$PG_ROLES_FILE"
echo "[$(date)] Roles and global objects dump saved to $PG_ROLES_FILE"

# ------------------------------------------------------------------
# 3️⃣ Копируем конфигурационные файлы PostgreSQL
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# 4️⃣ Очистка старых бэкапов (например, старше 7 дней)
# ------------------------------------------------------------------
